...
class libBayes(object):
    ...
    def train(self, y, x, preprocess=True):
        if preprocess:
            x = self.preprocess(x)
        self.get_set(x,y)
        #1. 获取p(y)
    ...
    def predict(self,samples, preprocess=True):
        '''
        预测函数
        '''
        if preprocess:
    ...
    def eval(self, y, x, preprocess=True):
        y_test = self.predict(x, preprocess=preprocess)
        diff = y_test - y
        correctN = np.argwhere(diff == 0.).shape[0]
        print('{}'.format(correctN/len(y)))

    def test(self, x, preprocess=True):
        pred_label = []
        y_test = self.predict(x, preprocess=preprocess)
        for i in range(0,len(y_test),100):
            count = np.bincount(y_test[i:i+100].astype(np.int64))
